...
interface ExtendedEDA {
  ...
  pcb_Drc?: {
    check: (strict: boolean, userInterface: boolean, includeVerboseError: boolean) => Promise<boolean | unknown[]>
  }
  sch_Drc?: {
    check: (strict: boolean, userInterface: boolean, includeVerboseError: boolean) => Promise<boolean | unknown[]>
  }
...
export async function edaQuery(): Promise<EDAQueryResult> {
  ...
  try {
    ...
    result.selection = {
      ...
      count: (pcbSelection?.length || 0) + (schSelection?.length || 0),
    }
  } catch {
  ...
  try {
    const pcbPassed = await eda.pcb_Drc?.check?.(true, false, false)
    const pcbErrors = pcbPassed === false ? await eda.pcb_Drc?.check?.(true, false, true) : undefined

    const schPassed = await eda.sch_Drc?.check?.(true, false, false)
    const schErrors = schPassed === false ? await eda.sch_Drc?.check?.(true, false, true) : undefined

    result.drc = {
      pcb: pcbPassed !== undefined ? { passed: Boolean(pcbPassed), errors: pcbErrors as unknown[] | undefined } : undefined,
      schematic: schPassed !== undefined ? { passed: Boolean(schPassed), errors: schErrors as unknown[] | undefined } : undefined,
    }
  } catch {
    result.drc = null
  }
